	...
	tests := []struct {
		...
		},
		{
			...
			operation: func(t *testing.T, tr *repository.TransferRepo, tc *testhelpers.TestContext, senderID, recipientID uint) error {
				...
				transfers, err := tr.GetPendingTransfersForRecipient(recipientID)
				if err != nil {
		...
		},
		{
			...
			operation: func(t *testing.T, tr *repository.TransferRepo, tc *testhelpers.TestContext, senderID, recipientID uint) error {
				...
				retrieved, err := tr.GetTransferByHash(txHash)
				if err != nil {
		...
		},
		{
			...
			operation: func(t *testing.T, tr *repository.TransferRepo, tc *testhelpers.TestContext, senderID, recipientID uint) error {
				...
				now := time.Now()
				err = tr.UpdateTransferStatus(transfer.ID, "claimed", &now)
				if err != nil {
					return fmt.Errorf("UpdateTransferStatus failed: %w", err)
				}

				// Verify status was updated
				retrieved, _ := tr.GetTransferByHash("tx_hash_update")
